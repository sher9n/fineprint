...
export function DivergenceTooltip({ divergenceScore, divergenceType, size = "md" }: { divergenceScore: number; divergenceType?: string; size?: "sm" | "md" }) {
  ...
  const chip = (
    <span className={cn("inline-flex items-center rounded-md border bg-[var(--bg-elev-2)] border-[var(--border)] cursor-default select-none whitespace-nowrap", containerSize)}>
      <span className={cn("uppercase tracking-wider text-[var(--text-dim)]", labelSize)}>Div</span>
  ...
  return (
    <Tooltip
      ...
      body={
        <>
          <p>How big the gap is between what the rules actually say and what most bettors assume from the question.</p>
